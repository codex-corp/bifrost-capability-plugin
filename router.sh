...
status() {
  ...
  echo "Runtime target: linux/amd64, Go 1.26.5, core v1.7.6"
  ...
  api GET /api/governance/routing-rules | jq -r '.rules[] | select(.name | startswith("Agent CR ")) | "  \(.priority) \(.name) enabled=\(.enabled)"'
}
...
backup_live() {
  ...
  for file in config.json config.db config.db-wal config.db-shm; do
    [[ -f "$BIFROST_APP_DIR/$file" ]] && cp -p "$BIFROST_APP_DIR/$file" "$backup_dir/$file"
  done
  api GET /api/plugins >"$backup_dir/plugins.json"
  api GET /api/governance/routing-rules >"$backup_dir/routing-rules.json"
  api GET /api/governance/complexity-analyzer-config >"$backup_dir/complexity-router.json"
}
...
upsert_rules() {
  ...
  existing="$(api GET /api/governance/routing-rules)"
  while IFS= read -r encoded; do
    rule="$(printf '%s' "$encoded" | base64 --decode)"
    name="$(jq -r '.name' <<<"$rule")"
    id="$(jq -r --arg name "$name" '.rules[] | select(.name == $name) | .id' <<<"$existing" | head -n1)"
    if [[ -n "$id" ]]; then
      api PUT "/api/governance/routing-rules/$id" -H 'Content-Type: application/json' --data "$rule" >/dev/null
    else
...
rollback() {
  ...
  while IFS= read -r id; do
    [[ -n "$id" ]] && api DELETE "/api/governance/routing-rules/$id" >/dev/null
